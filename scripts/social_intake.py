#!/usr/bin/env python3
"""
Goblin Recon - Social Signal Intake

Normalizes social media signals from approved APIs, public pages, or manual
assisted inputs into one schema before Trend Radar scoring.
"""
...
DEFAULT_STORE_PATH = ROOT / "vault" / "social-signals.jsonl"
...
def normalize_social_record(record: dict) -> dict:
    """Return a normalized social signal record with stable keys."""
    ...
    normalized["platform"] = _clean_text(normalized.get("platform")) or infer_platform(
        normalized["url"]
    )
...
def recommend_next_step(record: dict) -> str:
    ...
    if record.get("topic") and any(
        record.get(field) for field in ("caption", "hook", "format_type", "user_notes")
    ):
